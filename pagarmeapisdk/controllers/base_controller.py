...
class BaseController(object):
    ...
    def validate_response(self, response):
        ...
        if response.status_code == 400:
            raise ErrorException('Invalid request', response)
        elif response.status_code == 401:
            raise ErrorException('Invalid API key', response)
        elif response.status_code == 404:
            raise ErrorException('An informed resource was not found', response)
        elif response.status_code == 412:
            raise ErrorException('Business validation error', response)
        elif response.status_code == 422:
            raise ErrorException('Contract validation error', response)
        elif response.status_code == 500:
            raise ErrorException('Internal server error', response)
    ...
    def get_user_agent(self):
        user_agent = 'PagarmeCoreApi - Python 6.0.0-alpha.0'
        parameters = {
        }
